...
class Tokenizer(object):
    ...
    """
    _default_args = dict(lowercase=False, allcapskeep=True, normalize=3, 
                         usernames=None, urls='', phonenumbers=None, 
    ...
    del domains    
    # Regular expressions
    usernames_re = re.compile(r"@\w{1,15}",re.UNICODE)
    ...
    phonenumbers_re = re.compile(r"(?:\+?[01][\-\s\.]*)?(?:\(?\d{3}[\-\s\.\)]*)?\d{3}[\-\s\.]*\d{4}(?:\s*x\s*\d+)?"
                                 "(?=\s+|$)",re.UNICODE)
    numbers = r"(?:[+-]?\$?\d+(?:\.\d+)?(?:[eE]-?\d+)?%?)(?![A-Za-z])"
    numbers_re = re.compile(r"{0}(?:\s*/\s*{0})?".format(numbers), re.UNICODE)  # deals with fractions
    del numbers

    word_re = re.compile(r"(?:[a-zA-Z0-9]+['-]?[a-zA-Z]+[a-zA-Z0-9]*)|(?:[a-zA-Z0-9]*[a-zA-Z]+['-]?[a-zA-Z0-9]+)", re.UNICODE)
    ...
    tokenize_re = re.compile(ur"|".join(imap(lambda x: getattr(x, 'pattern', x),
                                             [usernames_re, urls_re, hashtags_re, 
                                              times_re, phonenumbers_re, numbers_re, 
                                              word_re, ellipsis_re, other_re])),re.UNICODE)

    repeating_re = re.compile(r"(.+?)\1\1+", re.UNICODE)
    repeating_words_re = re.compile(r"\b(.+?)( \1\b)+", re.UNICODE)
    repeating_symbols_re = re.compile(r"(\W+?)( \1)+",re.UNICODE)
    
    doublequotes = ((u'“',u'”'),(u'"',u'"'),(u'‘',u'’'),(u'＂',u'＂'))
    punctuation = (u'!$%()*+,-/:;<=>?[\\]^_.`{|}~\'' + u''.join(c for t in doublequotes for c in t))
    quotes_re = re.compile(ur"|".join(ur'({}.*?{})'.format(f,s) for f,s in doublequotes) + ur'|\s(\'.*?\')\s',re.UNICODE)
    del doublequotes
    ...
    def _replacetokens(self, msg):
        ...
        for word in msg:
            ...
            for token in self._token_regexs: # id & possibly replace tokens
                ...
                if regex.match(word):
                    if replacement_token is None: # keep the original word
                        tokens.append(self._cleanword(word))
                    else:
                        if replacement_token not in deletion_tokens: # decide if we change it
                ...
                if self.ellipsis_re.match(word):
                    ...
                    if self.emoji:
                        words=self._separate_emoticons_punctuation(word)
                        tokens.extend(words)
                    else:
                        tokens.append(word)
        return tokens
    ...
    def _separate_emoticons_punctuation(self, word):
        ...
        while i < len(word):
            ...
            for l in range(self._maxlenemo, 0, -1):
                ...
                if word[i] in self.punctuation and len(word)>6:
                    possibly_append_and_reset()
    ...
    def tokenize(self, message):
        ...
        message = self.repeating_words_re.sub(r'\1', message)
        message = self.repeating_symbols_re.sub(r'\1', message)
#        message = Tokenizer.strip_accents(message)
        message = self.tokenize_re.findall(message)
        message = self._replacetokens(message)
